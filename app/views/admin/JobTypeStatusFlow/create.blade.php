@extends('admin.layout')
	@section('body')
		<h3 class="page-title">Add Job Type Status Flow</h3>
			<!-- page layout -->
			<div class="page-bar">
				<ul class="page-breadcrumb">
					...
					<li>
						<a href="{{URL::route('admin.jobTypeStatusFlow.create')}}">Add Job Type Status Flow</a>
					</li>
				</ul>
			</div>
			<div class="row">
							    ...
							    <div class="form-body">
                                     @foreach ([
                                        ...
                                        'to_status_id' => 'To Status Name',
                                         ...
                                         @if ($key === 'job_type_id')
                                         <div class="form-group" id="jobTypeDiv">
                                              <label class="col-md-3 col-sm-3 col-xs-12 control-label">{{ Form::label($key, $value) }}</label>
                                               <div class="col-md-6 col-sm-6 col-xs-12" id="job_type_id">
                                                  {{Form::select($key, $jobType->lists('job_type_name','id'), null,  array('class' =>'form-control','onchange'=>'OnChangeJobType()','id'=>'jobTypeSelect')) }}
                                              </div>
                                         ...
                                         @elseif($key === 'channel_id')
                                            <div class="form-group" id="channelDiv">
                                                 <label class="col-md-3 col-sm-3 col-xs-12 control-label">{{ Form::label($key, $value) }}</label>
                                                  <div class="col-md-6 col-sm-6 col-xs-12" id="channel_id">
                                                    {{Form::select($key, $channels->lists('channel_name','id'), null,  array('class' =>'form-control','onchange'=>"onChangeChannel()",'id'=>'channelSelect')) }}
                                                 </div>
                                             </div>
                                         @elseif($key === 'from_status_id' || $key ==='to_status_id')
                                             <div class="form-group" id="{{$key}}">
                                                  <label class="col-md-3 col-sm-3 col-xs-12 control-label">{{ Form::label($key, $value) }}</label>
                                                   <div class="col-md-6 col-sm-6 col-xs-12" id="{{$key}}_list">
                                                    {{Form::select($key, $jobTypeStatus->lists('status_name','id'), null,  array('class' =>'form-control','id'=>$key."select")) }}
                                                  </div>
                                              </div>
                                         @endif
                                      @endforeach
